...
import { jobsByCategory } from '../../services/jobs/jobsByCategory'
...
export default function JobsPage({ jobs, page, numJobs,category, states }) {
 // console.log(category[0].name)
  ...
  const [verFiltrados, setVerFiltrados] =useState(false)
  const [filtrados,setFiltrados]=useState(jobs)

  useEffect(()=>{
    getAllJobs().then(newJobs=>{
      const filtro = newJobs.filter(job=>job.category.id == categoryId)
      console.log(filtro,newJobs,categoryId)
      setFiltrados(filtro)
    })
  },[categoryId])
  ...
  if(stateId){
    ...
    if(idCategory){
      jobsByCategory(idCategory).then(FilterJobs=>{
        setVerFiltrados(true)
      })
  ...
  return (
   ...
   <div  className={styles.filtro}>
            ...
            <div>
             <select className={styles.filtro} value={categoryId} onChange={e=>setCategoryId(e.target.value)}>
      ...
      <button onClick={() => router.push(`/jobs?page=${page + 1}`)}
        disabled={page >= lastPage} className={styles.next}>next</button></div>
